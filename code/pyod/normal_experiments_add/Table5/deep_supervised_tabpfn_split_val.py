...
for file_path in paths:
    ...
    train_label_pred = pfn_model.predict(X_train)
    test_label_pred = pfn_model.predict(X_test)
    ...
    print("*" * 100)
    pfn_model_noise = pfn_model
    train_label_pred_noise = pfn_model_noise.predict(X_train_copy)
    test_label_pred_noise = pfn_model_noise.predict(X_test_copy)
    ...
    predict_fn = lambda x: pfn_model_noise.predict_proba(x)
    ...
    pfn_repair = pfn_model
    y_train_pred = pfn_repair.predict(X_train_copy)
    y_test_pred = pfn_repair.predict(X_test_copy)
