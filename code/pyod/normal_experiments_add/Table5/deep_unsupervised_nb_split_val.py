...
def run(file_path):
    ...
    # 定义一个超参数网格
    param_grid = {
        'var_smoothing': [1e-9, 1e-8, 1e-7, 1e-6, 1e-5]  # 方差平滑参数，用于控制平滑强度
    }
    ...
    #
    # # 初始化GridSearchCV进行超参数调优，使用验证集进行交叉验证
    # grid_search = GridSearchCV(estimator=nb_model_noise, param_grid=param_grid,
    #                            cv=2,  # 3折交叉验证
    #                            scoring='accuracy',  # 使用准确度作为评分指标
    #                            n_jobs=-1,  # 使用所有CPU核进行并行计算
    #                            verbose=2)  # 打印详细的进度信息
    ...
    #
    # # 使用最佳超参数训练模型
    # nb_model_noise = grid_search.best_estimator_
    # nb_model_noise.fit(X_train_copy, y_train)
    nb_model_noise = nb_model
    ...
    #
    # # 初始化GridSearchCV进行超参数调优，使用验证集进行交叉验证
    # grid_search = GridSearchCV(estimator=nb_repair, param_grid=param_grid,
    #                            cv=2,  # 3折交叉验证
    #                            scoring='accuracy',  # 使用准确度作为评分指标
    #                            n_jobs=-1,  # 使用所有CPU核进行并行计算
    #                            verbose=2)  # 打印详细的进度信息
    ...
    #
    # # 使用最佳超参数训练模型
    # nb_repair = grid_search.best_estimator_
