...
for file_path in paths:
    ...
    # choice CART异常检测器
    # 初始化CART决策树模型
    # out_clf = DecisionTreeClassifier(random_state=42)
    # # 训练CART模型
    # out_clf.fit(X_train, y_semi)
    #
    # # 初始化第二个CART决策树模型
    # out_clf_noise = DecisionTreeClassifier(random_state=42)
    # # 训练第二个CART模型
    # out_clf_noise.fit(X_train_copy, y_semi)

    # choice ID3异常检测器
    # 初始化ID3模型
    out_clf = Id3Estimator()
    # 训练ID3模型
    out_clf.fit(X_train, y_semi)

    # 初始化第二个ID3决策树模型
    out_clf_noise = Id3Estimator()
    # 训练第二个ID3模型
    out_clf_noise.fit(X_train_copy, y_semi)
    ...
    test_label_pred = xgb_model.predict(X_test)
    ...
    """ROC-AUC指标"""
    y_test_prob = xgb_model_noise.predict_proba(X_test_copy)
    # 对于二分类任务
    ...
    predict_fn = lambda x: xgb_model_noise.predict_proba(x)
    ...
    """ROC-AUC指标"""
    y_test_prob = xgb_repair.predict_proba(X_test_copy)
    # 对于二分类任务
    roc_auc_test = roc_auc_score(y_test, y_test_prob[:, 1])  # 使用第二类的概率
    print("SVM模型在加噪测试集中的ROC-AUC分数：" + str(roc_auc_test))
    ...
    """Accuracy指标"""
    print("*" * 100)
    print("半监督异常检测器在修复测试集中的acccuracy：" + str(accuracy_score(y_test, y_test_pred)))

    """AP指标(不支持多分类)"""
    # 计算预测概率
    y_scores = xgb_repair.predict_proba(X_test_copy)
    # 计算 Average Precision
