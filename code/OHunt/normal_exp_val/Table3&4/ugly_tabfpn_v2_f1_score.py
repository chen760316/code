...
fpn_model.fit(X_train_copy, y_train)
...
predict_fn = lambda x: fpn_model.predict_proba(x)
...
y_mask = y.copy()
y_mask[test_indices] = fpn_model_noise.predict(X_test_copy)
# 对y_ground进行独热编码
encoder = OneHotEncoder(sparse_output=False)
y_true = encoder.fit_transform(y.reshape(-1, 1))

# 获取每棵树的预测概率 (n_samples, n_classes)
probabilities = fpn_model_noise.predict_proba(X_copy)
...
# 使用模型进行预测
y_pred_copy = fpn_model_noise.predict(X_copy)
# 计算分类错误率
