...
predict_fn = lambda x: svm_model_noise.predict_proba(x)
...
# 初始化MinMaxScaler
scaler = MinMaxScaler()
data_minmax = data_copy.copy().iloc[train_indices]
...
y_mask = y.copy()
y_mask[test_indices] = svm_model_noise.predict(X_test_copy)
# 对y_ground进行独热编码
...
# 使用模型进行预测
y_pred_train = svm_model_noise.predict(X_train_copy)
# 计算分类错误率
mis_classification_rate = 1 - accuracy_score(y_train, y_pred_train)
