...
# 首先，划分出训练集和临时集（临时集将用于验证集和测试集的划分）
...
# 初始化随机森林模型
rf_model_noise = RandomForestClassifier(random_state=42)

# 初始化GridSearchCV进行超参数调优，使用验证集进行交叉验证
grid_search = GridSearchCV(estimator=rf_model_noise, param_grid=param_grid,
                           cv=2,  # 5折交叉验证
                           scoring='accuracy',  # 使用准确度作为评分指标
                           n_jobs=-1,  # 使用所有CPU核进行并行计算
                           verbose=2)  # 打印详细的进度信息
