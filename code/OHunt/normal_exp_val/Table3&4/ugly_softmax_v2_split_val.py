...
X_train, X_temp, y_train, y_temp, train_indices, temp_indices = train_test_split(
    X, y, original_indices, test_size=0.5, random_state=1
)

# 然后从临时集（50%）中划分出验证集（2/5，即20%总数据集）和测试集（3/5，即30%总数据集）
X_val, X_test, y_val, y_test, val_indices, test_indices = train_test_split(
    X_temp, y_temp, temp_indices, test_size=0.6, random_state=1
)
...
grid_search = GridSearchCV(estimator=softmax_model, param_grid=param_grid,
                           cv=2,  # 5折交叉验证
                           scoring='accuracy',
                           n_jobs=-1,
                           verbose=2)
...
# 使用最佳超参数训练模型
softmax_model = grid_search.best_estimator_
...
# 使用最佳超参数训练模型
softmax_model_noise = grid_search.best_estimator_
...
y_mask = y.copy()
y_mask[test_indices] = softmax_model_noise.predict(X_test_copy)
# 对y_ground进行独热编码
...
# 使用模型进行预测
y_pred_train = softmax_model_noise.predict(X_train_copy)
# 计算分类错误率
