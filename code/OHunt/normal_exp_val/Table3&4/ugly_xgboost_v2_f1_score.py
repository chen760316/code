...
# file_path = "../datasets/synthetic_outlier/waveform_dependency_0.15.csv"
# file_path = "../datasets/synthetic_outlier/waveform_dependency_0.2.csv"
# file_path = "../datasets/synthetic_outlier/waveform_dependency_0.25.csv"
...
xgb_model = xgb.XGBClassifier(
    ...
    max_depth=6,                  # You can adjust the depth of trees
...
data_minmax = pd.DataFrame(scaler.fit_transform(data_minmax), columns=data_minmax.columns)
# 设置分组的间隔
interval = 0.01
interval_range = 5000
# 对每列数据进行分组
...
for i in top_k_indices:
    ...
    lower_threshold = median - interval_range * made
    upper_threshold = median + interval_range * made
...
# 初始化MinMaxScaler
scalar_new = MinMaxScaler()
data_imbalance = data_copy.copy()
data_imbalance = pd.DataFrame(scalar_new.fit_transform(data_imbalance), columns=data_imbalance.columns)
...
for feature in filtered_important_feature_indices:
    ...
    for t in X_copy_repair_indices:
        ...
        lower_threshold = median_imbalance - interval_range * made_imbalance
        upper_threshold = median_imbalance + interval_range * made_imbalance
