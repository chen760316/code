...
for file_path in paths:
    ...
    y_mask = y.copy()
    y_mask[test_indices] = fpn_model.predict(X_test_copy)
    # 对y_ground进行独热编码
    encoder = OneHotEncoder(sparse_output=False)
    y_true = encoder.fit_transform(y.reshape(-1, 1))

    # 获取每棵树的预测概率 (n_samples, n_classes)
    probabilities = fpn_model.predict_proba(X_copy)
    ...
    # 使用模型进行预测
    y_pred_copy = fpn_model.predict(X_copy)
    # 计算分类错误率
    ...
    y_pred = fpn_model.predict(X_copy)
    ...
    print("*" * 100)
    fpn_clf = TabPFNClassifier()
    fpn_clf.fit(X_train, y_train)
    train_label_pred = fpn_clf.predict(X_train)
    test_label_pred = fpn_clf.predict(X_test)
    ...
    print("*" * 100)
    train_label_pred_noise = fpn_clf.predict(X_train_copy)
    test_label_pred_noise = fpn_clf.predict(X_test_copy)
    ...
    fpn_repair = TabPFNClassifier()
    fpn_repair.fit(X_train_copy, y_train)
    y_train_pred = fpn_repair.predict(X_train_copy)
    y_test_pred = fpn_repair.predict(X_test_copy)
