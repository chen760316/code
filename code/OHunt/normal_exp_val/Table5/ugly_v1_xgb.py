...
for file_path in paths:
    ...
    xgb_model = xgb.XGBClassifier(
        objective='binary:logistic',  # Binary classification (use 'multi:softmax' for multi-class)
        eval_metric='logloss',  # Evaluation metric (log loss for binary classification)
        scale_pos_weight=1,  # To handle class imbalance (you can tune this)
        use_label_encoder=False,  # To avoid the label encoder warning
        booster='gbtree',  # Boosting type: 'gbtree' is commonly used for tabular data
        max_depth=3,  # You can adjust the depth of trees
    ...
    y_mask = y.copy()
    y_mask[test_indices] = xgb_model.predict(X_test_copy)
    # 对y_ground进行独热编码
    encoder = OneHotEncoder(sparse_output=False)
    y_true = encoder.fit_transform(y.reshape(-1, 1))

    # 获取每棵树的预测概率 (n_samples, n_classes)
    probabilities = xgb_model.predict_proba(X_copy)
    ...
    # 使用模型进行预测
    y_pred_copy = xgb_model.predict(X_copy)
    # 计算分类错误率
    ...
    y_pred = xgb_model.predict(X_copy)
    ...
    # xgb_model_noise.fit(X_train, y_train)
    xgb_model_noise = xgb_model
    train_label_pred = xgb_model_noise.predict(X_train)
    test_label_pred = xgb_model_noise.predict(X_test)
    ...
    test_label_pred_noise = xgb_model_noise.predict(X_test_copy)
    ...
    #
    # # 初始化 GridSearchCV
    # grid_search = GridSearchCV(estimator=xgb_repair, param_grid=param_grid,
    #                            scoring='accuracy',
    #                            cv=2,
    #                            verbose=2,
    #                            n_jobs=-1)
    ...
    #
    # # 获取最佳模型
    # xgb_repair = grid_search.best_estimator_
    # xgb_repair.fit(X_train_copy, y_train)
    xgb_repair = xgb_model
    y_train_pred = xgb_repair.predict(X_train_copy)
    y_test_pred = xgb_repair.predict(X_test_copy)
