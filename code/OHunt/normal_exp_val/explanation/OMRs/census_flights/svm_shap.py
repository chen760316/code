...
def sparsity(explainer, model, X_instance):
    ...
    # 将SHAP值大于0的特征作为重要特征
    selected_features = [feature for feature, value in zip(X_instance.columns, shap_values[0]) if abs(value) > 0.1]

    # 3. 计算稀疏性：选定特征的数量与总特征数量的比例
    total_features = X_instance.shape[1]
    selected_features_count = len(selected_features)
    sparsity_score = selected_features_count / total_features
    return sparsity_score


def fidelity(model, explainer, X_instance, num_features=5):
    """
    ...
    for idx in selected_features:
        modified_input[0, idx] = X_instance[0, idx]
