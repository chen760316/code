...
# choice census数据集
# file_path = "../large_datasets/census-income.csv"
# data = pd.read_csv(file_path)

# choice covtype数据集
# file_path = "../large_datasets/covtype_new.csv"
# data = pd.read_csv(file_path)
...
# label_name = "taxable income amount"
# 统计不同标签的数量
...
print(f"outliers占所有数据的比例: {min_ratio:.2%}")
...
# subsection census对应的处理

# # 假设 'label' 是数据集中包含标签的列名
#
# # 去除标签中的前后空格
# data[label_name] = data[label_name].str.strip()
#
# # 如果有其他特殊字符，使用 replace() 方法清理
# data[label_name] = data[label_name].str.replace(' ', '')  # 去除所有空格
#
# # 打印去除空格后的标签
# print(data[label_name].unique())
#
# if len(data) > 20000:
#     # 筛选出标签为 "-50000." 和 "50000+." 的数据
#     data_negative = data[data[label_name] == '-50000.']
#     data_positive = data[data[label_name] == '50000+.']
#
#     # 分别对这两个子集进行采样
#     data_negative_sampled = data_negative.sample(n=18758, random_state=42)
#     data_positive_sampled = data_positive.sample(n=1242, random_state=42)
#
#     # 合并这两个采样后的数据
#     data = pd.concat([data_negative_sampled, data_positive_sampled])
#
# data= data.dropna(axis=1, how='any')

# subsection flights对应的处理
data= data.dropna(axis=1, how='any')
...
print("离群值中good outliers比例：", len(good_outliers_index)/len(test_outliers_index))
print("离群值中bad outliers比例：", len(bad_outliers_index)/len(test_outliers_index))
print("bad outliers中ugly outliers的比例：", len(bad_wrong_indies)/len(bad_outliers_index))