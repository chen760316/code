...
test_label_pred = fpn_model.predict(X_test)
...
test_label_pred_noise = fpn_model_noise.predict(X_test_copy)
...
"""ROC-AUC指标"""
y_test_prob = fpn_model_noise.predict_proba(X_test_copy)
# 对于二分类任务
...
"""AP指标(不支持多分类)"""
# 计算预测概率
y_scores = fpn_model_noise.predict_proba(X_test_copy)
...
predict_fn = lambda x: fpn_model_noise.predict_proba(x)
...
"""ROC-AUC指标"""
y_test_prob = fpn_repair.predict_proba(X_test_copy)
# 对于二分类任务
roc_auc_test = roc_auc_score(y_test, y_test_prob[:, 1])  # 使用第二类的概率
print("SVM模型在加噪测试集中的ROC-AUC分数：" + str(roc_auc_test))
...
"""AP指标(不支持多分类)"""
# 计算预测概率
y_scores = fpn_repair.predict_proba(X_test_copy)
# 计算 Average Precision
ap_score = average_precision_score(y_test, y_scores[:, 1])
print("SVM模型在修复测试集中的AP分数:", ap_score)