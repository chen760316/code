...
initial_accuracy = (len(wrong_classified_train_indices_noise) + len(wrong_classified_test_indices_noise))/(len(y_train) + len(y_test))
...
# section 修复方案一：对X_copy中需要修复的元组进行标签修复（knn方法）
#  需要修复的元组通过异常值检测器检测到的元组和SVM分类错误的元组共同确定（取并集）

# subsection 尝试修复异常数据的标签
def knn_repair_label(X_copy_repair, X_copy_inners, y_inners, n_neighbors, X_train_copy, y, X_test_copy):
    ...
    svm_repair = svm.SVC(kernel='rbf', C=2.87, probability=True, class_weight='balanced')
    imputer = SimpleImputer(strategy='mean')  # 使用均值填充
    X_train_copy = imputer.fit_transform(X_train_copy)  # 返回的是numpy数组
    X_test_copy = imputer.fit_transform(X_test_copy)  # 返回的是numpy数组
    ...
    classification_accuracy = 1-(len(wrong_classified_train_indices) + len(wrong_classified_test_indices)) / (len(y_train) + len(y_test))
...
# section 修复方案二：对X_copy中需要修复的元组进行特征修复（统计方法修复）
#  需要修复的元组通过异常值检测器检测到的元组和SVM分类错误的元组共同确定（取并集）(修复效果由于监督/无监督基准)

# subsection 确定有影响力特征中的离群值并采用均值修复

def statistics_repair_data(X_copy, top_k_indices, y_train, X_copy_repair_indices):
...
def knn_repair_features(X_copy, top_k_indices, n_neighbors, train_indices, test_indices, y_train):
...
def delete_repair_data(X_copy_repair_indices, train_indices, X_copy, y, test_indices):
    ...
    svm_repair = svm.SVC(kernel='rbf', C=2.87, probability=True, class_weight='balanced')
    imputer = SimpleImputer(strategy='mean')  # 使用均值填充
    X_train_copy_repair = imputer.fit_transform(X_train_copy_repair)  # 返回的是numpy数组
    X_test_copy_repair = imputer.fit_transform(X_test_copy_repair)  # 返回的是numpy数组
...
def ml_repair_data(n_estimators, X_copy_inners, y_inners, X_copy_repair, y_repair):
    # subsection 修复标签值
    # 训练模型
...
def objective(trial):
    try:
        need_knn_label = trial.suggest_int("need_knn", low=0, high=1)
        need_statistics = trial.suggest_int("need_statistics", low=0, high=1)
        need_knn_features = trial.suggest_int("need_knn_features", low=0, high=1)
        need_delete = trial.suggest_int("need_delete", low=0, high=1)
        need_ml = trial.suggest_int("need_ml", low=0, high=1)
        knn_label_neighbors = trial.suggest_int("knn_label_neighbors", low=3, high=15)
        knn_features_neighbors = trial.suggest_int("knn_features_neighbors", low=3, high=15)
        n_estimators = trial.suggest_int("n_estimators", low=30, high=100)
        if need_knn_label == 1:
            accuracy = knn_repair_label(X_copy_repair, X_copy_inners, y_inners, knn_label_neighbors, X_train_copy, y, X_test_copy)
            return accuracy
        else:
            accuracy = initial_accuracy
        if need_statistics == 1:
            accuracy = statistics_repair_data(X_copy, top_k_indices, y_train, X_copy_repair_indices)
            return accuracy
        else:
            accuracy = initial_accuracy
        if need_knn_features == 1:
            accuracy = knn_repair_features(X_copy, top_k_indices, knn_features_neighbors, train_indices, test_indices, y_train)
            return accuracy
        else:
            accuracy = initial_accuracy
        if need_delete == 1:
            accuracy = delete_repair_data(X_copy_repair_indices, train_indices, X_copy, y, test_indices)
            return accuracy
        else:
            accuracy = initial_accuracy
        if need_ml == 1:
            accuracy = ml_repair_data(n_estimators, X_copy_inners, y_inners, X_copy_repair, y_repair)
            return accuracy
        else:
            accuracy = initial_accuracy
        return accuracy
        ...
        # 发生错误时，仍然抛出异常，让 optuna 继续进行后续的 trial
        raise e


storage_file_path = "./ugly_correction.log"
lock_obj = optuna.storages.journal.JournalFileOpenLock(storage_file_path)

storage = optuna.storages.JournalStorage(
    optuna.storages.journal.JournalFileBackend(storage_file_path, lock_obj=lock_obj),
)
