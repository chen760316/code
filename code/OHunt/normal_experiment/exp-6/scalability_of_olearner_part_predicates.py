...
file_path = "../large_datasets/samples/flights_20000.csv"
data = pd.read_csv(file_path)
...
def objective(trial):
    ...
    t_end_parameters = time.time()
    t_parameters = t_end_parameters - t_start_parameters
    ...
    if need_loss == 1:
        ...
        t_loss = t_end_loss - t_start_loss
    else:
    ...
    # t_end_others = time.time()
    # t_others = t_end_others - t_start_others
    t_train = t_MDetO + t_MDetI + t_outlier + t_imbalanced + t_loss + t_predicate
    print("训练时间为：", t_train)
    t_end = time.time()
    print("总的函数执行时间为：", t_end - t_start)
