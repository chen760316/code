...
with memory_timer():
    ...
    # 随机采样固定比例的行
    sample_size = 1.0  # 行采样比例
    data = data.sample(frac=sample_size)

    # 随机采样固定比例的列
    sample_ratio = 1.0  # 列采样比例
    ...
    # 随机选择特征列
    sampled_columns = data.columns[:-1].to_series().sample(n=num_sampled_features)
    ...
    print("*" * 100)
    svm_model_noise = RandomForestClassifier(n_estimators=50, max_depth=10, min_samples_split=10, min_samples_leaf=4, random_state=42, class_weight='balanced')
    ...
    def loss(clf, X_copy, X_test_copy, y_train, loss_choice):
        ...
        # choice 使用交叉熵损失函数(适用于二分类数据集下的通用分类器，判定bad不够准确)
            ...
            # 使用随机森林的 predict_proba 方法获取概率值
            y_pred_prob = clf.predict_proba(X_test_copy)  # (n_samples, n_classes)
            # 创建 OneHotEncoder 实例
    ...
    def objective(trial):
        ...
        # theta=5
        # outlier_detector = trial.suggest_categorical("outlier_detector", ["DeepSVDD", "RePEN"])
        outlier_detector = trial.suggest_categorical("outlier_detector", ["RePEN"])
        ...
        print("*" * 100)
        print("need_MDetO:", need_MDetO)
        print("need_MDetI:", need_MDetI)
        print("need_loss:", need_loss)
        print("need_outlier:", need_outlier)
        print("need_imbalanced:", need_imbalanced)
