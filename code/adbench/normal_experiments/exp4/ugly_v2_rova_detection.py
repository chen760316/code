...
out_clf.fit(X_train_copy, y_train)
...
for i in range(len(X_train)):
    if train_pred_labels[i] == y_train[i]:
...
for i in range(len(X_test)):
    if test_pred_labels[i] == y_test[i]:
...
for i in range(len(X_train_copy)):
    if train_pred_labels_noise[i] == y_train[i]:
...
for i in range(len(X_test_copy)):
    if test_pred_labels_noise[i] == y_test[i]:
